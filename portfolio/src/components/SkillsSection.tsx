...
export const SkillsSection = () => {
  ...
  Technologies I <span className="gradient-text">Work With</span>
          ...
          <p className="text-muted-foreground mt-4 max-w-4xl mx-auto text-2xl">
  ...
  {skillCategories.map((category) => (
    ...
    >
      <h3 className="font-display font-semibold text-2xl mb-4 gradient-text">
        {category.title}
      </h3>
      ...
      <div className="flex flex-wrap gap-2">
        {category.skills.map((skill) => (
          <span
            key={skill}
            className="px-3 py-1.5 text-base bg-white/5 border border-white/10 rounded-full text-muted-foreground hover:text-foreground hover:border-purple-500/50 transition-all duration-300 cursor-default"
          >
            {skill}
          </span>
        ))}
